	...
	<?php
		...
		foreach($_POST as $key=>$value){
			if(isset($_POST['way'])){
				if($_POST[$key]!=$_POST['way']){
					NettoyerAlphaNumerique($_POST[$key],$key,$erreur);
				}
				if($_POST[$key]==$_POST['way']){
					for($i=0;$i<count($_POST['way']);$i++){
						NettoyerAlphaNumerique($_POST['way'][$i],$_POST['way'][$i],$erreur);
					}
				}
			}
			if(!isset($_POST['way'])){
		...
		if($erreur>0){
			header("Location:Formulaire_Basique_stylise.php?".$PassageGET);
		}
